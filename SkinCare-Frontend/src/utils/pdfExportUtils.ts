...
export const exportOverviewToPDF = async (data: ExportData, includeCharts: boolean = true) => {
...
export const exportOverviewToPDFWithCharts = async (data: ExportData) => {
  const pdf = await exportOverviewToPDF(data, false);
  
  // Try to capture charts if they exist
  try {
    const chartElements = document.querySelectorAll('[data-chart]');
    let chartYPosition = 100;
    ...
    for (const chartElement of chartElements) {
      if (chartElement instanceof HTMLElement) {
        ...
        pdf.setFontSize(14);
        pdf.setFont('helvetica', 'bold');
        pdf.text('Sales Charts', 20, 20);
        
        pdf.addImage(imgData, 'PNG', 20, chartYPosition, imgWidth, imgHeight);
        chartYPosition += imgHeight + 20;
      }
    }
  } catch (error) {
